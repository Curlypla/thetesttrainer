...
def main():
    ...
    def preprocess_train(examples):
        ...
        for image in images:
            ...
            image = transforms.Resize((args.resolution_height, args.resolution_width), interpolation=transforms.InterpolationMode.BILINEAR)(image)
            if args.center_crop:
                y1 = max(0, int(round((image.height - args.resolution_height) / 2.0)))
                x1 = max(0, int(round((image.width - args.resolution_width) / 2.0)))
                image = transforms.functional.crop(image, y1, x1, args.resolution_height, args.resolution_width)
            else:
                y1, x1, h, w = transforms.RandomCrop.get_params(image, (args.resolution_height, args.resolution_width))
    ...
    def train_step(state, batch, train_rng):
        ...
        def compute_loss(params):
            ...
            def compute_time_ids(original_size, crops_coords_top_left):
                target_size = jnp.asarray((args.resolution_height, args.resolution_width))
